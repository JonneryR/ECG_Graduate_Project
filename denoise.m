...
points=12000; 
% 地址
PATH= 'E:\graduate_matlab\DL\data\MIT-BIH'; 
HEADERFILE= strcat(stringname,'.hea');     
ATRFILE= strcat(stringname,'.atr');        
DATAFILE=strcat(stringname,'.dat');        
SAMPLES2READ=points;         
...
%小波函数选择bior2.6，如果使用其他的需要进行调整
wavename='bior2.6';
level=8; 

ecgdata=M(:,1);
figure(2);
plot(ecgdata(1:points),'LineWidth',2);grid on ;axis tight;axis([1,points,-2,5]);
title('原始ECG信号——JonneryRMEMS');
...
figure(3);
plot(DenoisingSignal,'LineWidth',2);
title('小波基为bior2.6，去噪后的ECG信号'); grid on; axis tight;axis([1,points,-2,5]);
clear ecgdata;